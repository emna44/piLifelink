...
// Add to your server's index.js file
...
// Add these routes to your server's index.js file
...
// Get patient's complaints with status
...
app.get("/api/patients/:patientId/notifications", async (req, res) => {
    try {
        const { patientId } = req.params;
        const notifications = await NotificationModel.find({ recipient: patientId })
            .sort({ createdAt: -1 });
